...
export default {
    ...
    updated() {
        ...
        const newChildren = Array.from(this.$el.children);
        const added = newChildren.filter(c => !this._oldChidren.includes(c));
        const removed = this.removedKeys.map(key => Array.from(this.$el.children).find(c => c.__vue__.$vnode.key === key));
    ...
    methods: {
        ...
        buildFilterFunction(name) {
            ...
            this._filterlistener = this.$watch(() => { return this.list.map((el, index) => this.options.getFilterData[name](el, index)); },
                () => { this.iso._requestUpdate(); });
            return filter;
        },
